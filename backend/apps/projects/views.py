    ...
    def perform_create(self, serializer):
        ...
        if not wbs_code:
            ...
            if parent is not None:
                # Sous-tâche : numérotée SOUS son parent → {wbs_parent}.{séquence}
                # (ex. 1.1 → 1.1.1, 1.1.2). On ne reprend pas le code de phase.
                base = parent.wbs_code
                existing_count = Task.objects.filter(
                    project=project, parent=parent
                ).count()
            else:
                # Tâche racine : {code_phase}.{séquence} (ex. 1.1, 1.2).
                phase = serializer.validated_data.get("phase")
                base = phase.code if phase and phase.code else str(phase.pk if phase else "0")
